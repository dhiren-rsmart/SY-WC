...
namespace smART.Library {
  public abstract class AttachmentLibrary<TEntityBusiness, TEntityModel, TParentEntityBusiness, TParentEntityModel> : GenericLibrary<TEntityBusiness, TEntityModel>, IParentChildLibrary<TEntityBusiness>
    where TEntityBusiness : VModel.AttachmentEntity<TParentEntityBusiness>, new()
    where TEntityModel : Model.AttachmentEntity<TParentEntityModel>, new()
    where TParentEntityBusiness : VModel.BaseEntity, new()
    where TParentEntityModel : Model.BaseEntity, new() {
    public AttachmentLibrary() : base() {
    }
    public AttachmentLibrary(string dbContextConnectionString) : base(dbContextConnectionString) {
    }
